...
export function parseObjects(params: ParseObjectsParams): ParseObjectsResult {
  ...
  while (
    ...
    startsWith(lines[currentLine1], " ", position)
  ) {
    const startLine = currentLine1;
    console.info(
      `Parsing object ${field.name} at line ${currentLine1} with value ${lines[currentLine1]}`
    );
    ...
    console.info(
      `Parsed object ${field.name} from line ${startLine} to line ${currentLine1}`
    );
