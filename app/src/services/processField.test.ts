...
function processLines(currentLine: number, lines: string[], pod: Pod) {
    while (currentLine < lines.length) {
        ...
        podFields.forEach(field => {
            ...
            if (!isEmpty(obj)) {
                pod = {...pod, ...obj};
                console.info(`Parsed field ${field.name} with value ${JSON.stringify(obj)} starting at line ${currentLine} and ending at line ${endLine}`);
...
describe("processField", () => {
    ...
    it("should return the correct value and endLine", () => {
        ...
        expect(pod).toEqual({name: "my-pod", namespace: "default", priority: "0", node: "minikube"});
    });
